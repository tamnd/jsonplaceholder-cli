// Package cli assembles the jsonplaceholder command tree from the jsonplaceholder
...
// NewApp assembles the kit application from the jsonplaceholder domain. The
...
//
// To add a command, declare it in jsonplaceholder/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := jsonplaceholder.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(jsonplaceholder.Domain{}).Register(app)
