...
describes what users get. Pick a format with `-o`, or let jsonplaceholder choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jsonplaceholder <command> -o table   # aligned columns for reading
jsonplaceholder <command> -o jsonl   # one JSON object per line, for piping
jsonplaceholder <command> -o json    # a single JSON array
jsonplaceholder <command> -o csv     # spreadsheet friendly
jsonplaceholder <command> -o tsv     # tab-separated
jsonplaceholder <command> -o url     # just the URL column
jsonplaceholder <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jsonplaceholder <command> --fields id,title,url
```
...
```bash
jsonplaceholder <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jsonplaceholder <command>            # a table, because this is a terminal
jsonplaceholder <command> | wc -l    # JSONL, because this is a pipe
```
